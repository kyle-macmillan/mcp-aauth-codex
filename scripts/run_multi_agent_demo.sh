#!/usr/bin/env bash
set -euo pipefail

plugin_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"
state_dir="${plugin_dir}/.demo-state"
infra_log="${state_dir}/infra.log"
infra_launcher="${plugin_dir}/scripts/run_infra.sh"
agent_launcher="${plugin_dir}/scripts/run_new_agent.sh"
session_name="edocs-demo"
client="codex"

if [[ ${1:-} == "--client" ]]; then
  if [[ $# -lt 2 ]]; then
    echo "--client requires codex or claude" >&2
    exit 2
  fi
  client=$2
  shift 2
fi
if [[ "${client}" != "codex" && "${client}" != "claude" ]]; then
  echo "Unsupported coding-agent client: ${client}" >&2
  exit 2
fi

if ! command -v tmux >/dev/null 2>&1; then
  echo "tmux is required for the multi-agent launcher." >&2
  exit 1
fi
if tmux has-session -t "${session_name}" 2>/dev/null; then
  echo "tmux session ${session_name} already exists." >&2
  echo "Attach with: tmux attach -t ${session_name}" >&2
  exit 1
fi

mkdir -p -m 700 "${state_dir}"
: > "${infra_log}"
"${infra_launcher}" > "${infra_log}" 2>&1 &
infra_pid=$!

cleanup() {
  kill "${infra_pid}" 2>/dev/null || true
  wait "${infra_pid}" 2>/dev/null || true
}
trap cleanup EXIT INT TERM

for _ in $(seq 1 100); do
  [[ -f "${state_dir}/ready" ]] && break
  kill -0 "${infra_pid}" 2>/dev/null || {
    echo "Demo services exited before becoming ready." >&2
    tail -n 20 "${infra_log}" >&2
    exit 1
  }
  sleep 0.1
done
if [[ ! -f "${state_dir}/ready" ]]; then
  echo "Timed out waiting for demo services." >&2
  tail -n 20 "${infra_log}" >&2
  exit 1
fi

tmux new-session -d -s "${session_name}" -n agents \
  "\"${agent_launcher}\" \"${client}\" producer --agent-id aauth:producer@demo.local --person alice --display-name Producer"
tmux split-window -h -t "${session_name}:agents" \
  "\"${agent_launcher}\" \"${client}\" carol --agent-id aauth:carol@demo.local --person carol --display-name Carol"
tmux split-window -v -t "${session_name}:agents.1" \
  "\"${agent_launcher}\" \"${client}\" bob --agent-id aauth:bob@demo.local --person bob --display-name Bob"
tmux select-layout -t "${session_name}:agents" tiled
tmux select-pane -t "${session_name}:agents.0"

echo "Multi-agent demo ready."
echo "Client:   ${client}"
echo "Producer: aauth:producer@demo.local"
echo "Carol:    aauth:carol@demo.local"
echo "Bob:      aauth:bob@demo.local"
echo "Control panel: http://127.0.0.1:8721/demo"
tmux attach-session -t "${session_name}"
